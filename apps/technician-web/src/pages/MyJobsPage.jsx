...
export default function MyJobsPage({jobs,onOpen,onCreate}){
 ...
 return <>
  <div className="page-title"><div><h1>งานของฉัน</h1><p>เลือกงานเพื่อเริ่มบันทึกหน้างาน</p></div><button className="primary compact" onClick={onCreate}>+ งานติดตั้ง</button></div>
  <div className="tabs">
   <button className={tab==='active'?'active':''} onClick={()=>setTab('active')}>งานปัจจุบัน</button>
   <button className={tab==='completed'?'active':''} onClick={()=>setTab('completed')}>เสร็จแล้ว</button>
