...
export default function ProblemCasesPage({problems,onClaim}){
 ...
 return <>
  <div className="page-title"><div><h1>เคสปัญหาทั้งหมด</h1><p>เลือกเคสที่ยังไม่มีผู้รับผิดชอบเพื่อสร้างเป็นงานซ่อมของฉัน</p></div></div>
  ...
  <div className="case-list">{visible.map(item=><article className="case-card" key={item.problem_id}>
   ...
   {item.problem_status==='open'?<button className="primary full" disabled={claiming===item.problem_id} onClick={()=>claim(item)}>{claiming===item.problem_id?'กำลังรับเคส...':'รับเคสนี้'}</button>:item.job_id&&<div className="case-linked">อยู่ในงานของฉัน · งาน #{item.job_id}</div>}
