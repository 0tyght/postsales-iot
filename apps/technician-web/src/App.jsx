...
export default function App({session=null,onSessionLogout}){
 ...
 const[jobs,setJobs]=useState([]),[problems,setProblems]=useState([]),[sites,setSites]=useState([]),[customers,setCustomers]=useState([]),[models,setModels]=useState([]),[devices,setDevices]=useState([]);
 const[selected,setSelected]=useState(null),[creating,setCreating]=useState(false),[error,setError]=useState('');
 const load=useCallback(()=>Promise.all([api('/jobs'),api('/problems'),api('/customer-sites'),api('/customers'),api('/devices/models'),api('/devices/units')]).then(([jobData,problemData,siteData,customerData,modelData,deviceData])=>{setJobs(jobData);setProblems(problemData);setSites(siteData);setCustomers(customerData);setModels(modelData);setDevices(deviceData);setError('')}).catch(x=>setError(x.message)),[]);
 ...
 return <TechnicianLayout user={user} onLogout={logout}>
  ...
  <TechnicianRoutes jobs={jobs} problems={problems} customers={customers} sites={sites} models={models} devices={devices} selected={selected} onOpen={setSelected} onBack={()=>setSelected(null)} onCreate={()=>setCreating(true)} onChanged={load} onClaim={async problemId=>{await api(`/problems/${problemId}/claim`,{method:'POST'});await load()}}/>
