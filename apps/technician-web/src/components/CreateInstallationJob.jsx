...
export default function CreateInstallationJob({sites,onClose,onDone}){
 ...
 return <div className="backdrop"><form className="sheet" onSubmit={submit}>
  <div className="sheet-head"><h2>สร้างงานติดตั้งใหม่</h2><button type="button" className="icon" onClick={onClose}>×</button></div>
  <p>ใช้สำหรับงานติดตั้งครั้งแรกหรือเพิ่มจุดติดตั้งใหม่ ช่างเลือก/เพิ่มลูกค้าก่อน แล้วค่อยบันทึกอุปกรณ์จริงในหน้างาน</p>{error&&<div className="alert error">{error}</div>}
