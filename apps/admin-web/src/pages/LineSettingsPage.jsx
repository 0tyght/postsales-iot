...
export default function LineSettingsPage(){
 ...
 const send=async e=>{e.preventDefault();setSending(true);setError('');setMessage('');try{await api('/line/push',{method:'POST',body:JSON.stringify(form)});setMessage('ส่งข้อความทดสอบแล้ว');setForm({...form,text:''})}catch(x){setError(x.message)}finally{setSending(false)}};
 return <section>
  <div className="page-head"><div><h1>LINE ลูกค้า</h1><p>ผูก LINE ของลูกค้ากับข้อมูลในระบบ เพื่อรับแจ้งปัญหาและส่งอัปเดตให้ถูกคน</p></div></div>
  ...
  <div className="dashboard-grid">
   ...
   <div className="panel">
    ...
    <form onSubmit={send}>
     ...
     <label className="form-field">ข้อความ<textarea rows="5" value={form.text} onChange={e=>setForm({...form,text:e.target.value})} required placeholder="เช่น แจ้งนัดหมาย หรือแจ้งสถานะงาน"/></label>
