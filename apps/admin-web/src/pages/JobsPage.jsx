...
function JobForm({row,lookups,onClose,onSaved}){
 const isEdit=Boolean(row?.job_id),defaults={job_type:'repair',job_status:'created',customer_mode:'existing',site_status:'active',service_interval_days:30};
 ...
 const fields=[
  {name:'job_type',label:'ประเภทงาน',type:'select',required:true,hideOnEdit:true,options:[{value:'repair',label:'งานบริการจากเคส'},{value:'installation',label:'งานติดตั้งจุดใหม่'}]},
  {name:'problem_id',label:'เคสบริการ',type:'select',required:true,hideOnEdit:true,showWhen:f=>f.job_type==='repair',options:lookups.problems?.filter(x=>!x.job_id&&x.problem_status==='open').map(x=>({value:x.problem_id,label:`#${x.problem_id} · ${x.customer_name} · ${x.site_name}`}))||[]},
  {name:'site_id',label:'จุดติดตั้ง',type:'select',required:true,showWhen:()=>isEdit,options:lookups.sites?.filter(x=>x.site_status==='active').map(x=>({value:x.site_id,label:`${x.customer_name} · ${x.site_name}`}))||[]},
  {name:'customer_mode',label:'ลูกค้า',type:'select',required:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation',options:[{value:'existing',label:'เลือกลูกค้าเดิม'},{value:'new',label:'เพิ่มลูกค้าใหม่'}]},
  {name:'customer_id',label:'เลือกลูกค้าเดิม',type:'select',required:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation'&&f.customer_mode!=='new',options:lookups.customers?.map(x=>({value:x.customer_id,label:`${x.customer_name} · ${x.phone||'-'}`}))||[]},
  {name:'customer_name',label:'ชื่อลูกค้าใหม่',required:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation'&&f.customer_mode==='new'},
  {name:'phone',label:'โทรศัพท์ลูกค้าใหม่',required:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation'&&f.customer_mode==='new'},
  {name:'email',label:'อีเมลลูกค้าใหม่',type:'email',hideOnEdit:true,showWhen:f=>f.job_type==='installation'&&f.customer_mode==='new'},
  {name:'address',label:'ที่อยู่ติดต่อของลูกค้าใหม่',type:'textarea',fullWidth:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation'&&f.customer_mode==='new'},
  {name:'site_name',label:'ชื่อจุดติดตั้งใหม่',required:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation',placeholder:'เช่น บ้านหลัง A, สาขาพิษณุโลก'},
  {name:'site_address',label:'ที่อยู่หน้างาน',type:'textarea',fullWidth:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation'},
  {name:'service_start_date',label:'วันที่เริ่มดูแล',type:'date',hideOnEdit:true,showWhen:f=>f.job_type==='installation'},
  {name:'service_end_date',label:'วันที่สิ้นสุดระยะดูแล',type:'date',hideOnEdit:true,showWhen:f=>f.job_type==='installation'},
  {name:'service_interval_days',label:'ติดต่อ/ตรวจทุกกี่วัน',type:'number',required:true,hideOnEdit:true,showWhen:f=>f.job_type==='installation'},
  {name:'next_service_contact_date',label:'กำหนดติดต่อ/ตรวจครั้งถัดไป',type:'date',hideOnEdit:true,showWhen:f=>f.job_type==='installation'},
 ...
 const save=async e=>{e.preventDefault();setSaving(true);setError('');try{let payload={...form};if(!isEdit&&form.job_type==='installation'){let customerId=form.customer_id;if(form.customer_mode==='new'){const customer=await api('/customers',{method:'POST',body:JSON.stringify({customer_name:form.customer_name,phone:form.phone,email:form.email,address:form.address})});customerId=customer.customer_id}const site=await api('/customer-sites',{method:'POST',body:JSON.stringify({customer_id:customerId,site_name:form.site_name,site_address:form.site_address,site_status:'active',service_start_date:form.service_start_date,service_end_date:form.service_end_date,service_interval_days:form.service_interval_days,next_service_contact_date:form.next_service_contact_date})});payload={job_type:'installation',job_status:'created',site_id:site.site_id,technician_id:form.technician_id,scheduled_at:form.scheduled_at,job_note:form.job_note||'งานติดตั้งใหม่จากการสร้างจุดติดตั้งใหม่'}}await api(isEdit?`/jobs/${row.job_id}`:'/jobs',{method:isEdit?'PUT':'POST',body:JSON.stringify(payload)});onSaved()}catch(x){setError(x.message)}finally{setSaving(false)}};
...
export default function JobsPage(){
 const lookups=useLookups({sites:'/customer-sites',customers:'/customers',users:'/users',problems:'/problems'});
