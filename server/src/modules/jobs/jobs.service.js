...
exports.list=r.list;
const statusText={created:'สร้างงานแล้ว',in_progress:'ช่างกำลังดำเนินงาน',completed:'ดำเนินงานเสร็จแล้ว',cancelled:'ยกเลิกงาน'};
...
exports.create=async(data,actor)=>{if(actor.role==='technician'&&data.job_type!=='installation'){const e=new Error('ช่างสร้างได้เฉพาะงานติดตั้ง งานซ่อมต้องมาจากแอดมิน');e.status=403;throw e;}const id=await r.create({...data,technician_id:actor.role==='technician'?actor.user_id:data.technician_id},actor.user_id);await notify(await r.notificationContext(id));return id;};
