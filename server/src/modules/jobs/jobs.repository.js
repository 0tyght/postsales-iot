...
exports.create=async(d,userId)=>{const cn=await db.getConnection();try{await cn.beginTransaction();let siteId=d.site_id,scheduledAt=d.scheduled_at||null,jobNote=d.job_note||null;
 if(d.job_type==='repair'){if(!d.problem_id){const e=new Error('งานบริการต้องสร้างจากเคสบริการ');e.status=400;throw e;}const[[problem]]=await cn.query('SELECT site_id,job_id,problem_status,preferred_appointment_at,preferred_appointment_note FROM problem_reports WHERE problem_id=? FOR UPDATE',[d.problem_id]);if(!problem){const e=new Error('ไม่พบเคสบริการ');e.status=404;throw e;}if(problem.job_id||problem.problem_status!=='open'){const e=new Error('เคสนี้ถูกรับหรือปิดไปแล้ว กรุณารีเฟรชรายการ');e.status=409;throw e;}siteId=problem.site_id;scheduledAt=scheduledAt||problem.preferred_appointment_at||null;jobNote=jobNote||problem.preferred_appointment_note||null;}
 ...
 VALUES (?,?,?,?,?,?,?,?,?)`,[siteId,d.technician_id||null,d.job_status||'created',scheduledAt,jobNote,d.result_summary||null,d.started_at||null,d.completed_at||null,userId]);
